...
/**
  ...
  * @retval Status
  */
uint8_t USBH_MSC_TestUnitReady (USB_OTG_CORE_HANDLE *pusbdev)
{
  uint8_t index;
  ...
  {  
    ...
    case CMD_SEND_STATE:  
      ...
      USBH_MSC_CBWData.field.CBWFlags = USB_EPT_DIR_OUT;
...
/**
  ...
  * @retval Status
  */
uint8_t USBH_MSC_ReadCapacity10(USB_OTG_CORE_HANDLE *pusbdev)
{
  uint8_t index;
  USBH_MSC_Status_TypeDef status = USBH_MSC_BUSY;
  ...
  {  
    ...
    case CMD_SEND_STATE:
      ...
      USBH_MSC_CBWData.field.CBWFlags = USB_EPT_DIR_IN;
...
/**
  ...
  * @retval Status
  */
uint8_t USBH_MSC_ModeSense6(USB_OTG_CORE_HANDLE *pusbdev)
{
  uint8_t index;
  USBH_MSC_Status_TypeDef status = USBH_MSC_BUSY;
...
/**
  ...
  * @retval Status
  */
uint8_t USBH_MSC_RequestSense(USB_OTG_CORE_HANDLE *pusbdev)
{
  USBH_MSC_Status_TypeDef status = USBH_MSC_BUSY;
...
/**
  ...
  * @retval Status
  */
uint8_t USBH_MSC_Write10(USB_OTG_CORE_HANDLE *pusbdev, 
                         uint8_t *dataBuffer,
                         uint32_t address,
                         uint32_t nbOfbytes)
{
...
/**
  ...
  * @retval Status
  */
uint8_t USBH_MSC_Read10(USB_OTG_CORE_HANDLE *pusbdev,
                        uint8_t *dataBuffer,
                        uint32_t address,
                        uint32_t nbOfbytes)
{
  ...
  if(USB_HCD_IsDeviceConnected(pusbdev))
  {
    ...
    case CMD_WAIT_STATUS:
      ...
      if((USBH_MSC_BOTXferParam.BOTXferStatus == USBH_MSC_OK) && \
        (USB_HCD_IsDeviceConnected(pusbdev)))
      { 
        /* Commands successfully sent and Response Received  */       
        USBH_MSC_BOTXferParam.CmdStateMachine = CMD_SEND_STATE;
        status = USBH_MSC_OK;      
