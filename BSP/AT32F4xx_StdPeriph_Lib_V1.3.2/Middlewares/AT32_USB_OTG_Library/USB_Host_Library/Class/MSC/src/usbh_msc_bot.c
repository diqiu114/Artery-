...
* @retval None
*/
void USBH_MSC_Init(USB_OTG_CORE_HANDLE *pusbdev )
{
...
* 
*/
void USBH_MSC_HandleBOTXfer (USB_OTG_CORE_HANDLE *pusbdev ,USB_HOST *phost)
{
  ...
  {  
    ...
    case USBH_MSC_SENT_CBW:
      URB_Status = USB_HCD_GetURB_State(pusbdev , MSC_Machine.hch_num_out);
      ...
      { 
        ...
        xferDirection = (USBH_MSC_CBWData.field.CBWFlags & USB_REQUEST_DIR_MASK);
    ...
    case USBH_MSC_BOT_DATAIN_STATE:
      ...
      URB_Status =   USB_HCD_GetURB_State(pusbdev , MSC_Machine.hch_num_in);
      /* BOT DATA IN stage */
      if((URB_Status == URB_DONE) ||(USBH_MSC_BOTXferParam.BOTStateBkp != USBH_MSC_BOT_DATAIN_STATE))
      {
        ...
        {
          USB_HOST_BulkReceiveData (pusbdev,
	                        datapointer, 
    ...
    case USBH_MSC_DECODE_CSW:
      ...
      if(URB_Status == URB_DONE)
      {
        ...
        USBH_MSC_BOTXferParam.BOTXferStatus = USBH_MSC_DecodeCSW(pusbdev , phost);
      }
      else if(URB_Status == URB_STALL)     
      {
        error_direction = USBH_MSC_DIR_IN;
    ...
    case USBH_MSC_BOT_ERROR_IN: 
      status = USBH_MSC_BOT_Abort(pusbdev, phost, USBH_MSC_DIR_IN);
      ...
      else if (status == USB_HOST_UNRECOVERED_ERROR)
      {
    ...
    case USBH_MSC_BOT_ERROR_OUT: 
      status = USBH_MSC_BOT_Abort(pusbdev,phost, USBH_MSC_DIR_OUT);
      if ( status == USB_HOST_OK)
...
* @retval None
*/
USB_HOST_Status USBH_MSC_BOT_Abort(USB_OTG_CORE_HANDLE *pusbdev, 
                               USB_HOST *phost,
                               uint8_t direction)
{
  USB_HOST_Status status;
...
uint8_t USBH_MSC_DecodeCSW(USB_OTG_CORE_HANDLE *pusbdev , USB_HOST *phost)
{
  uint8_t status;
  ...
  if(USB_HCD_IsDeviceConnected(pusbdev))
  {
    ...
    dataXferCount = USB_HCD_GetXferCnt(pusbdev, MSC_Machine.hch_num_in); 
