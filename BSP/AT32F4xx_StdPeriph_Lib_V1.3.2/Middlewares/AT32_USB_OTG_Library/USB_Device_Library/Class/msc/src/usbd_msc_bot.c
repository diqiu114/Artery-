...
void MSC_BOT_Init (USB_OTG_CORE_HANDLE  *pdev)
{
  ...
  USB_DCD_EPT_Flush(pdev, MSC_OUT_EP);
  USB_DCD_EPT_Flush(pdev, MSC_IN_EP);
  /* Prapare EP to Receive First BOT Cmd */
  USB_DCD_EPT_PrepareRx (pdev,
                    MSC_OUT_EP,
                    (uint8_t *)&MSC_BOT_cbw,
                    BOT_CBW_LENGTH);    
}
...
static void  MSC_BOT_CBW_Decode (USB_OTG_CORE_HANDLE  *pdev)
{
  ...
  if ((USB_Device_GetRxCount (pdev ,MSC_OUT_EP) != BOT_CBW_LENGTH) ||
                              ...
                              uint16_t len)
{
  ...
  USB_DCD_EPT_Tx (pdev, MSC_IN_EP, buf, len);  
}
...
static void  MSC_BOT_Abort (USB_OTG_CORE_HANDLE  *pdev)
{
  ...
  if ((MSC_BOT_cbw.bmFlags == 0) && 
      ...
      (MSC_BOT_Status == BOT_STATE_NORMAL) )
  {
    USB_DCD_EPT_Stall(pdev, MSC_OUT_EP );
  }
  USB_DCD_EPT_Stall(pdev, MSC_IN_EP);
