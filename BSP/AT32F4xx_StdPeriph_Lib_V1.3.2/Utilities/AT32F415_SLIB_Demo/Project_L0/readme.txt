...
@par Example Description 
  ...
  Because the AT32F415 does not have FPU, so the software floating-point library will be used for floating-point arithmetic operation.
  
  However, some functions in the prebuilt library have literal-pool. So these functions can not be placed in the sLib protected area.

  These functions are then placed in a fixed memory area and packaged as a binary file. Please refer to the application note for more detailed 
  ...
  it (avoiding literal pools and branch tables), and all thes used software floting-point library functions are packaged into a binary file.
  
  
  @note The IP-Code and IP-Data are placed in page 116 to page 121 (address 0x0803A000 ~ 0x0803CFFF) which will be protected.
...
The used ARM software floating-point library functions
  - Project_L0/MDK_V5/C_Lib_Bin/ER_C_LIB.bin             File containing the used library functions
  - Project_L0/MDK_V5/C_Lib_Bin/slib_used_c_lib.s        Assembly code that includes the "ER_C_LIB.bin". It is used for Project_L1.  
  
     
@par Hardware and Software environment  

  - This example runs on AT32F415RCT7 devices.
  - This example has been tested with AT-START-F415 V1.0 board.
