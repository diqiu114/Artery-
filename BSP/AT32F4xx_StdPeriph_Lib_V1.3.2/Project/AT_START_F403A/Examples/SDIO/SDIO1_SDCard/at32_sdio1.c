/**
  ******************************************************************************
  * File   : SDIO/SDIO1_SDCard/at32_sdio1.c
  * Version: V1.3.2
  * Date   : 2021-08-08
...
SD_Error SDIO_SendDataCommand(SDIO_CmdInitType *SDIO_CmdInitStruct, SDIO_DataInitType* SDIO_DataInitStruct, u32 *buf)
{
  ...
  if(DeviceMode == SD_POLLING_MODE) /* 轮询模式 */
  {
    if (SDIO_DataInitStruct->SDIO_TransferDir == SDIO_TransferDir_ToSDIO)
    {
      ...
      {
        if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_RXBUF_H) != RESET)
        {
      ...
      while(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_RXBUF) != RESET)
      {
        *buf = SDIO1->BUF;
        buf++;
      }
    ...
    else
    {
      ...
      {
        if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_TXBUF_H) != RESET)
        {
    ...
    if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_DTTIMEOUT) != RESET)   //数据超时
    {
      SDIO_ClearFlag(SDIO1, SDIO_FLG_DTTIMEOUT);
      return SD_DATA_TIMEOUT;
    }
    else if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_DTFAIL) != RESET) //CRC错误
    {
      SDIO_ClearFlag(SDIO1, SDIO_FLG_DTFAIL);
      return SD_DATA_FAIL;
    }
    else if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_RXERRO) != RESET) //上溢错误
    {
      SDIO_ClearFlag(SDIO1, SDIO_FLG_RXERRO);
      return SD_RX_OVERRUN;
    }
    else if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_TXERRU) != RESET) //下溢错误
    {
      SDIO_ClearFlag(SDIO1, SDIO_FLG_TXERRU);
      return SD_TX_UNDERRUN;
    }
    else if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_SBITERR) != RESET) //起始位错误
    {
      SDIO_ClearFlag(SDIO1, SDIO_FLG_SBITERR);
      return SD_START_BIT_ERR;
    }

    if(StopCondition == 1 && SDIO_GetFlagStatus(SDIO1, SDIO_FLG_DTCMPL) != RESET) //接收结束
    {
...
SD_Error SD_ProcessIRQSrc(void)
{
  ...
  if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_DTFAIL) != RESET) //数据CRC错误
  {
    SDIO_ClearFlag(SDIO1, SDIO_FLG_DTFAIL); //清错误标志
    SDIO1->INTEN &= ~((1 << 1) | (1 << 3) | (1 << 8) | (1 << 14) | (1 << 15) | (1 << 4) | (1 << 5) | (1 << 9));
    TransferError = SD_DATA_FAIL;
    return(SD_DATA_FAIL);
  }

  if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_DTTIMEOUT) != RESET) //数据超时错误
  {
    SDIO_ClearFlag(SDIO1, SDIO_FLG_DTTIMEOUT); //清中断标志
    SDIO1->INTEN &= ~((1 << 1) | (1 << 3) | (1 << 8) | (1 << 14) | (1 << 15) | (1 << 4) | (1 << 5) | (1 << 9)); //关中断
    TransferError = SD_DATA_TIMEOUT;
    return(SD_DATA_TIMEOUT);
  }

  if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_RXERRO) != RESET) //FIFO上溢错误
  {
    SDIO_ClearFlag(SDIO1, SDIO_FLG_RXERRO);              //清中断标志
    SDIO1->INTEN &= ~((1 << 1) | (1 << 3) | (1 << 8) | (1 << 14) | (1 << 15) | (1 << 4) | (1 << 5) | (1 << 9)); //关中断
    TransferError = SD_RX_OVERRUN;
    return(SD_RX_OVERRUN);
  }

  if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_TXERRU) != RESET) //FIFO下溢错误
  {
    SDIO_ClearFlag(SDIO1, SDIO_FLG_TXERRU);              //清中断标志
    SDIO1->INTEN &= ~((1 << 1) | (1 << 3) | (1 << 8) | (1 << 14) | (1 << 15) | (1 << 4) | (1 << 5) | (1 << 9)); //关中断
    TransferError = SD_TX_UNDERRUN;
    return(SD_TX_UNDERRUN);
  }

  if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_SBITERR) != RESET) //起始位错误
  {
    SDIO_ClearFlag(SDIO1, SDIO_FLG_SBITERR); //清中断标志
    SDIO1->INTEN &= ~((1 << 1) | (1 << 3) | (1 << 8) | (1 << 14) | (1 << 15) | (1 << 4) | (1 << 5) | (1 << 9)); //关中断
...
SD_Error CmdError(void)
{
  ...
  while(timeout--)
  {
    if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_CMDCMPL) != RESET)
    {
      break;
    }
  }
...
SD_Error CmdResp7Error(void)
{
  ...
  if((timeout == 0) || (status & (1 << 2))) //响应超时
  {
    ...
    SDIO_ClearFlag(SDIO1, SDIO_FLG_CMDTIMEOUT);
    return errorstatus;
  }
  ...
  {
    errorstatus = SD_OK;
    SDIO_ClearFlag(SDIO1, SDIO_FLG_CMDRSPCMPL);
  }
...
SD_Error CmdResp1Error(u8 cmd)
{
  ...
  if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_CMDTIMEOUT) != RESET) //响应超时
  {
    SDIO_ClearFlag(SDIO1, SDIO_FLG_CMDTIMEOUT);
    return SD_CMD_RSP_TIMEOUT;
  }

  if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_CMDFAIL) != RESET)   //CRC错误
  {
    SDIO_ClearFlag(SDIO1, SDIO_FLG_CMDFAIL);
    return SD_CMD_FAIL;
  }
...
SD_Error IsCardProgramming(u8 *pstatus)
{
  ...
  if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_CMDFAIL) != RESET)   //CRC检测失败
  {
    SDIO_ClearFlag(SDIO1, SDIO_FLG_CMDFAIL);
    return SD_CMD_FAIL;
  }

  if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_CMDTIMEOUT) != RESET)    //命令超时
  {
    SDIO_ClearFlag(SDIO1, SDIO_FLG_CMDTIMEOUT);
    return SD_CMD_RSP_TIMEOUT;
  }
...
SD_Error FindSCR(void)
{
  ...
  while(!(SDIO1->STS & (SDIO_FLG_RXERRO | SDIO_FLG_DTFAIL | SDIO_FLG_DTTIMEOUT | SDIO_FLG_DTBLKCMPL | SDIO_FLG_SBITERR)))
  {
    if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_RXBUF) != RESET) //接收FIFO数据可用
    {
  ...
  else if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_DTFAIL) != RESET)   //数据块CRC错误
  {
    SDIO_ClearFlag(SDIO1, SDIO_FLG_DTFAIL);
    return SD_DATA_FAIL;
  }
  else if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_RXERRO) != RESET)   //接收fifo上溢错误
  {
    SDIO_ClearFlag(SDIO1, SDIO_FLG_RXERRO);
    return SD_RX_OVERRUN;
  }
  else if(SDIO_GetFlagStatus(SDIO1, SDIO_FLG_SBITERR) != RESET)  //接收起始位错误
  {
    SDIO_ClearFlag(SDIO1, SDIO_FLG_SBITERR);
    return SD_START_BIT_ERR;
  }
