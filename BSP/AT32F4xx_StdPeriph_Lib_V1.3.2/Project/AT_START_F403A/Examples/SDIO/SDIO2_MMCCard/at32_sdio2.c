...
SD_Error SD_Init(void)
{
  ...
  RCC_AHBPeriphClockCmd(RCC_AHBPERIPH_SDIO2, ENABLE); //使能SDIO2时钟
  RCC_APB2PeriphClockCmd(RCC_APB2PERIPH_GPIOC | RCC_APB2PERIPH_GPIOA, ENABLE); //使能GPIOA, GPIOC时钟
...
SD_Error SD_PowerON(void)
{
  ...
  SDIO_SetPowerSaveState(SDIO2, SDIO_PowerSave_ON); //上电状态,开启卡时钟

  SDIO_ClockCmd(SDIO2, ENABLE); //SDIOCK使能
  ...
  for(i = 0; i < 10; i++)
  {
    ...
    SDIO_SendCommand(SDIO2, &SDIO_CmdInitStructure);    //写命令进命令寄存器
  ...
  SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
  SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
  SDIO_SendCommand(SDIO2, &SDIO_CmdInitStructure);  //发送CMD55,短响应
...
{
  SDIO_SetPowerSaveState(SDIO2, SDIO_PowerSave_OFF); //SDIO电源关闭,时钟停止
  return SD_OK;
}
...
SD_Error SD_InitializeCards(void)
{
  ...
  if(SDIO_GetPowerSaveState(SDIO2) == 0) //检查电源状态,确保为上电状态
  {
  ...
  if(SDIO_SECURE_DIGITAL_IO_CARD != CardType)   //非SECURE_DIGITAL_IO_CARD
  {
    ...
    SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
    SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
    SDIO_SendCommand(SDIO2, &SDIO_CmdInitStructure); //发送CMD2,取得CID,长响应
      ...
      (SDIO_SECURE_DIGITAL_IO_COMBO_CARD == CardType) || (SDIO_HIGH_CAPACITY_SD_CARD == CardType))          //判断卡类型
  {
    ...
    SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
    SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
    SDIO_SendCommand(SDIO2, &SDIO_CmdInitStructure);    //发送CMD3,短响应
...
SD_Error SD_EnableWideBusOperation(u32 wmode)
{
  ...
  {
    SDIO2->CLKCTRL &= ~(3 << 11);   //清除之前的位宽设置
    SDIO2->CLKCTRL |= (u16)wmode << 11; //设置总线宽度
  }
...
SD_Error SD_SelectDeselect(u32 addr)
{
  ...
  SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
  SDIO_SendCommand(SDIO2, &SDIO_CmdInitStructure); //发送CMD7,选择卡,短响应
...
SD_Error SDIO_SendDataCommand(SDIO_CmdInitType *SDIO_CmdInitStruct, SDIO_DataInitType* SDIO_DataInitStruct, u32 *buf)
{
  ...
  SDIO2->DTCTRL = 0x0; //数据控制寄存器清零(关DMA)
  SDIO_DataConfig(SDIO2, SDIO_DataInitStruct);
  SDIO_SendCommand(SDIO2, SDIO_CmdInitStruct); //发送CMD17+从addr地址出读取数据,短响应
  ...
  if(DeviceMode == SD_POLLING_MODE) /* 轮询模式 */
  {
    ...
    if(SDIO_GetFlagStatus(SDIO2, SDIO_FLG_DTTIMEOUT) != RESET)  //数据超时
    {
      SDIO_ClearFlag(SDIO2, SDIO_FLG_DTTIMEOUT);
      return SD_DATA_TIMEOUT;
    }
    else if(SDIO_GetFlagStatus(SDIO2, SDIO_FLG_DTFAIL) != RESET) //CRC错误
    {
      SDIO_ClearFlag(SDIO2, SDIO_FLG_DTFAIL);
      return SD_DATA_FAIL;
    }
    else if(SDIO_GetFlagStatus(SDIO2, SDIO_FLG_RXERRO) != RESET) //上溢错误
    {
      SDIO_ClearFlag(SDIO2, SDIO_FLG_RXERRO);
      return SD_RX_OVERRUN;
    }
    else if(SDIO_GetFlagStatus(SDIO2, SDIO_FLG_TXERRU) != RESET) //下溢错误
    {
      SDIO_ClearFlag(SDIO2, SDIO_FLG_TXERRU);
      return SD_TX_UNDERRUN;
    }
    else if(SDIO_GetFlagStatus(SDIO2, SDIO_FLG_SBITERR) != RESET) //起始位错误
    {
      SDIO_ClearFlag(SDIO2, SDIO_FLG_SBITERR);
      return SD_START_BIT_ERR;
    }

    if(StopCondition == 1 && SDIO_GetFlagStatus(SDIO2, SDIO_FLG_DTCMPL) != RESET) //接收结束
    {
    ...
    SDIO_ClearFlag(SDIO2, SDIO_STATIC_FLAGS); //清除所有标记
  }
...
SD_Error SD_EraseBlocks(long long addr, u32 nblks)
{
  ...
  if(SDIO2->RSP1 & SD_CARD_LOCKED) //卡锁了
  {
...
SD_Error SD_ReadBlock(u8 *readbuff, long long ReadAddr, u16 BlockSize)
{
  ...
  if((BlockSize > 0) && (BlockSize <= 2048) && ((BlockSize & (BlockSize - 1)) == 0))
  {
    ...
    SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
    SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
    SDIO_SendCommand(SDIO2, &SDIO_CmdInitStructure); //发送CMD16+设置数据长度为blksize,短响应
...
SD_Error SD_ProcessIRQSrc(void)
{
  if(SDIO2->STS & (1 << 8)) //接收完成中断
  {
    ...
    SDIO2->INTCLR |= 1 << 8;
    SDIO2->INTEN &= ~((1 << 1) | (1 << 3) | (1 << 8) | (1 << 14) | (1 << 15) | (1 << 4) | (1 << 5) | (1 << 9)); //关中断
    TransferEnd = 1;
    return(TransferError);
  }

  if(SDIO_GetFlagStatus(SDIO2, SDIO_FLG_DTFAIL) != RESET) //数据CRC错误
  {
    SDIO_ClearFlag(SDIO2, SDIO_FLG_DTFAIL); //清错误标志
    SDIO2->INTEN &= ~((1 << 1) | (1 << 3) | (1 << 8) | (1 << 14) | (1 << 15) | (1 << 4) | (1 << 5) | (1 << 9));
    TransferError = SD_DATA_FAIL;
    return(SD_DATA_FAIL);
  }

  if(SDIO_GetFlagStatus(SDIO2, SDIO_FLG_DTTIMEOUT) != RESET) //数据超时错误
  {
    SDIO_ClearFlag(SDIO2, SDIO_FLG_DTTIMEOUT); //清中断标志
    SDIO2->INTEN &= ~((1 << 1) | (1 << 3) | (1 << 8) | (1 << 14) | (1 << 15) | (1 << 4) | (1 << 5) | (1 << 9)); //关中断
    TransferError = SD_DATA_TIMEOUT;
    return(SD_DATA_TIMEOUT);
  }

  if(SDIO_GetFlagStatus(SDIO2, SDIO_FLG_RXERRO) != RESET) //FIFO上溢错误
  {
    SDIO_ClearFlag(SDIO2, SDIO_FLG_RXERRO);             //清中断标志
    SDIO2->INTEN &= ~((1 << 1) | (1 << 3) | (1 << 8) | (1 << 14) | (1 << 15) | (1 << 4) | (1 << 5) | (1 << 9)); //关中断
    TransferError = SD_RX_OVERRUN;
    return(SD_RX_OVERRUN);
  }

  if(SDIO_GetFlagStatus(SDIO2, SDIO_FLG_TXERRU) != RESET) //FIFO下溢错误
  {
    SDIO_ClearFlag(SDIO2, SDIO_FLG_TXERRU);             //清中断标志
    SDIO2->INTEN &= ~((1 << 1) | (1 << 3) | (1 << 8) | (1 << 14) | (1 << 15) | (1 << 4) | (1 << 5) | (1 << 9)); //关中断
    TransferError = SD_TX_UNDERRUN;
    return(SD_TX_UNDERRUN);
  }

  if(SDIO_GetFlagStatus(SDIO2, SDIO_FLG_SBITERR) != RESET) //起始位错误
  {
    SDIO_ClearFlag(SDIO2, SDIO_FLG_SBITERR); //清中断标志
    SDIO2->INTEN &= ~((1 << 1) | (1 << 3) | (1 << 8) | (1 << 14) | (1 << 15) | (1 << 4) | (1 << 5) | (1 << 9)); //关中断
...
SD_Error CmdResp1Error(u8 cmd)
{
  ...
  if(SDIO_GetFlagStatus(SDIO2, SDIO_FLG_CMDTIMEOUT) != RESET) //响应超时
  {
    SDIO_ClearFlag(SDIO2, SDIO_FLG_CMDTIMEOUT);
    return SD_CMD_RSP_TIMEOUT;
  }

  if(SDIO_GetFlagStatus(SDIO2, SDIO_FLG_CMDFAIL) != RESET)  //CRC错误
  {
...
SD_Error CmdResp6Error(u8 cmd, u16*prca)
{
  ...
  if(SDIO2->RSPCMD != cmd) //判断是否响应cmd命令
  {
    return SD_ILLEGAL_CMD;
  }
...
SD_Error SDEnWideBus(u8 enx)
{
  ...
  if(SDIO2->RSP1 & SD_CARD_LOCKED) //SD卡处于LOCKED状态
  {
    return SD_LOCK_UNLOCK_FAILED;
  }
...
SD_Error IsCardProgramming(u8 *pstatus)
{
  ...
  {
    status = SDIO2->STS;  //等待操作完成
  }

  if(SDIO_GetFlagStatus(SDIO2, SDIO_FLG_CMDFAIL) != RESET)  //CRC检测失败
  {
    SDIO_ClearFlag(SDIO2, SDIO_FLG_CMDFAIL);
    return SD_CMD_FAIL;
  }

  if(SDIO_GetFlagStatus(SDIO2, SDIO_FLG_CMDTIMEOUT) != RESET)   //命令超时
  {
    SDIO_ClearFlag(SDIO2, SDIO_FLG_CMDTIMEOUT);
    return SD_CMD_RSP_TIMEOUT;
  }
...
SD_Error FindSCR(void)
{
  ...
  {
    if(SDIO_GetFlagStatus(SDIO2, SDIO_FLG_RXBUF) != RESET) //接收FIFO数据可用
    {
      *(tempscr + index) = SDIO_ReadData(SDIO2);    //读取FIFO内容
      index++;
    }
  }
  ...
  else if(SDIO_GetFlagStatus(SDIO2, SDIO_FLG_DTFAIL) != RESET)  //数据块CRC错误
  {
    SDIO_ClearFlag(SDIO2, SDIO_FLG_DTFAIL);
    return SD_DATA_FAIL;
  }
  else if(SDIO_GetFlagStatus(SDIO2, SDIO_FLG_RXERRO) != RESET)  //接收fifo上溢错误
  {
    SDIO_ClearFlag(SDIO2, SDIO_FLG_RXERRO);
    return SD_RX_OVERRUN;
  }
  else if(SDIO_GetFlagStatus(SDIO2, SDIO_FLG_SBITERR) != RESET)     //接收起始位错误
  {
    SDIO_ClearFlag(SDIO2, SDIO_FLG_SBITERR);
    return SD_START_BIT_ERR;
  }
...
void SD_DMA_Config(u32*mbuf, u32 bufsize, u32 DMA_DIR)
{
  ...
  DMA_Reset(DMA2_Channel5); //复位DMA2通道5
  DMA_ChannelEnable(DMA2_Channel5, DISABLE); //关闭DMA2通道5

  DMA_InitStructure.DMA_PeripheralBaseAddr = (u32)&SDIO2->BUF;  //DMA外设基地址
  ...
  DMA_ChannelEnable(DMA2_Channel5, ENABLE); //开启DMA2 通道5
}
