/**
  ******************************************************************************
  * File   : SDIO/SDIO_MMCCard/at32_sdio.c
  * Version: V1.3.2
  * Date   : 2021-08-08
...
SD_Error SD_Init(void)
{
  ...
  RCC_AHBPeriphClockCmd(RCC_AHBPERIPH_SDIO1, ENABLE); //使能SDIO时钟
  RCC_APB2PeriphClockCmd(RCC_APB2PERIPH_GPIOA | RCC_APB2PERIPH_GPIOC | RCC_APB2PERIPH_GPIOD, ENABLE); //使能GPIOA, GPIOC时钟

  /* ---Remap_0100--- */
#ifdef SDIO_REMAP0100
  RCC_APB2PeriphClockCmd(RCC_APB2PERIPH_AFIO, ENABLE);
  GPIO_PinsRemapConfig(AFIO_MAP6_SDIO_0100, ENABLE);
  ...
  /* ---Remap_0101--- */
#elif defined SDIO_REMAP0101
  RCC_APB2PeriphClockCmd(RCC_APB2PERIPH_AFIO, ENABLE);
  GPIO_PinsRemapConfig(AFIO_MAP6_SDIO_0101, ENABLE);
  ...
  /* ---Remap_0110--- */
#elif defined SDIO_REMAP0110
  RCC_APB2PeriphClockCmd(RCC_APB2PERIPH_AFIO, ENABLE);
  GPIO_PinsRemapConfig(AFIO_MAP6_SDIO_0110, ENABLE);
  ...
  /* ---Remap_0111--- */
#elif defined SDIO_REMAP0111
  RCC_APB2PeriphClockCmd(RCC_APB2PERIPH_AFIO, ENABLE);
  GPIO_PinsRemapConfig(AFIO_MAP6_SDIO_0111, ENABLE);
  ...
  /* ---No Remap--- */
#else
  GPIO_InitStructure.GPIO_Pins = GPIO_Pins_6 | GPIO_Pins_7 | GPIO_Pins_8 | GPIO_Pins_9 | GPIO_Pins_10 | GPIO_Pins_11 | GPIO_Pins_12; //PC6-->D6, PC7-->D7, PC8~PC11-->D0~D3, PC12-->CK
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;               //复用推挽输出
  GPIO_InitStructure.GPIO_MaxSpeed = GPIO_MaxSpeed_50MHz;       //IO口速度为50MHz
  GPIO_Init(GPIOC, &GPIO_InitStructure);                        //根据设定参数初始化
  
  GPIO_InitStructure.GPIO_Pins = GPIO_Pins_8 | GPIO_Pins_9;     //PB8-->D4, PB9-->D5
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;               //复用推挽输出
  GPIO_InitStructure.GPIO_MaxSpeed = GPIO_MaxSpeed_50MHz;       //IO口速度为50MHz
  GPIO_Init(GPIOB, &GPIO_InitStructure);                        //根据设定参数初始化

  GPIO_InitStructure.GPIO_Pins = GPIO_Pins_2;                   //PD2-->CMD
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;               //复用推挽输出
  GPIO_InitStructure.GPIO_MaxSpeed = GPIO_MaxSpeed_50MHz;       //IO口速度为50MHz
