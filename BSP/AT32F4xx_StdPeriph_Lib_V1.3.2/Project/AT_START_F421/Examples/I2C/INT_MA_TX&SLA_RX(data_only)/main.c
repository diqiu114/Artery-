/**
  ******************************************************************************
  * File   : I2C/INT_MA_TX&SLA_RX(data_only)/main.c 
  * Version: V1.3.2
  * Date   : 2021-08-08
...
/** @addtogroup INT_MA_TX&SLA_RX(data_only)
  * @{
  */ 
...
int main(void)
{
  ...
  printf("Interrupt - Master Transmitter & Slave Receiver(data only).\r\n");  
  ...
  while(1)
  {
    ...
    if((I2C_Status = I2C_Master_Transmit_INT(I2C_PORT, I2C_SLAVE_ADDRESS7, tx_buf, BUF_SIZE, 10000)) != I2C_OK)
    {
    ...
    /* Timeout is set to 10S  */
    if((I2C_Status = I2C_Slave_Receive_INT(I2C_PORT, rx_buf, BUF_SIZE, 10000)) != I2C_OK)
    {
      /* Error_Handler() function is called when error occurs */ 
      Error_Handler(I2C_Status);
    }
