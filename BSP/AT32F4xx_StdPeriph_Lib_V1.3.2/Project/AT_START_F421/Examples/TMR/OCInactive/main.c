...
int main(void)
{
  ...
  /* ---------------------------------------------------------------
    ...
    The objective is to get TMR3 counter clock at 1 KHz:
     - Prescaler = (TIM2CLK / TMR3 counter clock) - 1
    And generate 4 signals with 4 different delays:
    TMR2_CH1 delay = CCR1_Val/TMR3 counter clock = 1000 ms
    TMR2_CH2 delay = CCR2_Val/TMR3 counter clock = 500 ms
    TMR2_CH3 delay = CCR3_Val/TMR3 counter clock = 250 ms
    TMR2_CH4 delay = CCR4_Val/TMR3 counter clock = 125 ms
  --------------------------------------------------------------- */
  ...
  GPIO_SetBits(GPIOB, GPIO_Pins_6 | GPIO_Pins_7 | GPIO_Pins_8 | GPIO_Pins_9);
