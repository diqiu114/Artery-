...
void TMR3_GLOBAL_IRQHandler(void)
{
  ...
  {
    /* Clear TMR3 Capture Compare1 interrupt pending bit*/
  ...
  {
    /* Clear TMR3 Capture Compare2 interrupt pending bit*/
    TMR_ClearITPendingBit(TMR3, TMR_INT_CC2);

    /* PC.07 turnoff after 500 ms */
    GPIO_ResetBits(GPIOB, GPIO_Pins_7);
  }
  else if (TMR_GetINTStatus(TMR3, TMR_INT_CC3) != RESET)
  {
    /* Clear TMR3 Capture Compare3 interrupt pending bit*/
    TMR_ClearITPendingBit(TMR3, TMR_INT_CC3);

    /* PC.08 turnoff after 250 ms */
    GPIO_ResetBits(GPIOB, GPIO_Pins_8);
  }
  else
  {
    /* Clear TMR3 Capture Compare4 interrupt pending bit*/
    TMR_ClearITPendingBit(TMR3, TMR_INT_CC4);

    /* PC.09 turnoff after 125 ms */
    GPIO_ResetBits(GPIOB, GPIO_Pins_9);
  }
}
