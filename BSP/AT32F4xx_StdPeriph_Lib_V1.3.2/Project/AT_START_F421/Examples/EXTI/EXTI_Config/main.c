...
int main(void)
{
  ...
  EXTI_GenerateSWInt(EXTI_Line17); ///<Generate software interrupt: simulate a falling edge applied on EXTI22 line
	while (1)
	{
	}
}
...
void EXTI0_Config(void)
{
	...
	GPIO_Init(GPIOA, &GPIO_InitStructure);                       ///<Configure PA.00 pin as input 
	
	RCC_APB2PeriphClockCmd(RCC_APB2PERIPH_SYSCFGCOMP, ENABLE);         ///<Enable SYSCFG clock
	SYSCFG_EXTILineConfig(GPIO_PortSourceGPIOA, GPIO_PinsSource0); ///<Connect EXTI0 Line to PA0 pin
...
void EXTI9_5_Config(void)
{
	...
	GPIO_Init(GPIOB, &GPIO_InitStructure);                        ///<Configure PB9 pin as input 
	
	RCC_APB2PeriphClockCmd(RCC_APB2PERIPH_SYSCFGCOMP, ENABLE);    ///<Enable SYSCFG clock
	SYSCFG_EXTILineConfig(GPIO_PortSourceGPIOB, GPIO_PinsSource9);  ///<Connect EXTI9 Line to PB9 pin
