/**
  ******************************************************************************
  * File   : I2C/Poll_PEC_MA_RX&SLA_TX/main.c 
  * Version: V1.3.2
  * Date   : 2021-08-08
...
/** @addtogroup Poll_PEC_MA_RX&SLA_TX
  * @{
  */ 
...
int main(void)
{
  ...
  printf("Polling(PEC) - Master Receiver & Slave Transmitter.\r\n");  
  ...
  while(1)
  {
    ...
    if((I2C_Status = I2C_Master_Receive(I2C_PORT, I2C_SLAVE_ADDRESS7, rx_buf, BUF_SIZE, 10000)) != I2C_OK)
    {
    ...
    else
    {
      ...
      printf("PEC value: %x ", rx_buf[BUF_SIZE - 1]);     
    }
    ...
    /* Timeout is set to 10S  */
    if((I2C_Status = I2C_Slave_Transmit(I2C_PORT, tx_buf, BUF_SIZE - 1, 10000)) != I2C_OK)
    {
      /* Error_Handler() function is called when error occurs */ 
      Error_Handler(I2C_Status);
    }
