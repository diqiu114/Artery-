...
const uint16_t I2S1_Buffer_Tx[32] = {0x0102, 0x0304, 0x0506, 0x0708, 0x090A, 0x0B0C,
...
int main(void)
{
  ...
  DMA_InitStructure.DMA_MemoryBaseAddr = (uint32_t)&I2S1_Buffer_Tx;
  ...
  TransferStatus = Buffercmp(I2S2_Buffer_Rx, (uint16_t*)I2S1_Buffer_Tx, 32);
  /* TransferStatus = PASSED, if the data transmitted from I2S1 and received by
                               I2S2 are the same
     TransferStatus = FAILED, if the data transmitted from I2S1 and received by
                               I2S2 are different */
...
void RCC_Configuration(void)
{
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2PERIPH_GPIOA | RCC_APB2PERIPH_GPIOB | RCC_APB2PERIPH_GPIOF | RCC_APB2PERIPH_SPI1 |
