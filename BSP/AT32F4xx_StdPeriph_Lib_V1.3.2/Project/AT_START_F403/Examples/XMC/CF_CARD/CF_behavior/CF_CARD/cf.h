/*
  ...
  * Version: V1.3.2
  * Date   : 2021-08-08
  * Brief  : CF card operation header file.
  **************************************************************************
  */
#ifndef __CF_H
#define __CF_H
...
#define FALSE 	0
#define TRUE	1
#define XMC_BASE             			((uint32_t)0x60000000) /*!< XMC base address */
#define XMC_BANK4            			(XMC_BASE + 0x30000000)  /*!< XMC Bank4 base address */
#define PCCARD_DEVICE_ADDRESS           XMC_BANK4  /* 0x9000 0000 */
#define PCCARD_COMMON_SPACE_ADDRESS     PCCARD_DEVICE_ADDRESS                  /* Common space size to @0x93FF FFFF    */
#define PCCARD_ATTRIBUTE_SPACE_ADDRESS  ((uint32_t)(XMC_BANK4 + 0x08000000))  /* Attribute space size to @0x9BFF FFFF */
#define PCCARD_IO_SPACE_ADDRESS         ((uint32_t)(XMC_BANK4 + 0x0C000000))  /* IO space size to @0x9FFF FFFF        */

// In XMC document page 545/1128, the XMC is not supported True IDE mode. //
#define PCCARD_IO_SPACE_PRIMARY_ADDR    ((uint32_t)(XMC_BANK4 + PCCARD_IO_SPACE_ADDRESS + 0x1F0))  /* IO space size to @0x9FFF FFFF        */
#define PCCARD_IO_SPACE_SECONDARY_ADDR 	((uint32_t)(XMC_BANK4 + PCCARD_IO_SPACE_ADDRESS + 0x170))  /* IO space size to @0x9FFF FFFF        */
...
//void (*InterruptHandler)();
  ...
  typedef struct
  {
    ...
    XMC_PCCARDInitType       Init;                   /*!< PCCARD device control configuration parameters   */
...
#define __PCCARD_Reset_HANDLE_STATE(__HANDLE__) ((__HANDLE__)->State = PCCARD_STATE_RESET)
  ...
  void PCCARD_MspInit(PCCARD_HandleType *hpccard);
  void PCCARD_MspDeInit(PCCARD_HandleType *hpccard);
...
#define XMC_BKxCTRL_DEV_PCCARD        ((uint32_t)0x00000008)
