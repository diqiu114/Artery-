...
/*!< INT2 NAND pin configuration */  
  ...
  XMC_NANDInitStructure.XMC_EnableWait = XMC_WaitOperation_Disable;
  XMC_NANDInitStructure.XMC_BusType = XMC_BusType_8b;
  XMC_NANDInitStructure.XMC_EnableECC = XMC_ECCOperation_Disable;
  XMC_NANDInitStructure.XMC_ECCPageSize = XMC_ECCPageSize_2048Bytes;
...
uint32_t NAND_WriteSmallPage(uint8_t *pBuffer, NAND_ADDRESS Address, uint32_t NumPageToWrite)
{
  ...
  while((NumPageToWrite != 0x00) && (addressstatus == NAND_VALID_ADDRESS) && (status == NAND_READY))
  {
    ...
    *(__IO uint8_t *)(Bank_NAND_ADDR | CMD_AREA) = NAND_CMD_WRITE_TRUE1;
    ...
    while(XMC_GetFlagStatus(XMC_Bank2_NAND,XMC_FLAG_FEMPT)!=SET);
    ECC_Value_Write=XMC_GetECC(XMC_Bank2_NAND);
    XMC_NANDECCCmd(XMC_Bank2_NAND, DISABLE);
...
uint32_t NAND_ReadSmallPage(uint8_t *pBuffer, NAND_ADDRESS Address, uint32_t NumPageToRead)
{
  ...
  while((NumPageToRead != 0x0) && (addressstatus == NAND_VALID_ADDRESS))
  {
    ...
    *(__IO uint8_t *)(Bank_NAND_ADDR | CMD_AREA) = NAND_CMD_AREA_TRUE1;         
    ...
    ECC_Value_Read=XMC_GetECC(XMC_Bank2_NAND);
    XMC_NANDECCCmd(XMC_Bank2_NAND, DISABLE);
