/**
  ******************************************************************************
  * File   : I2C/Poll_PEC_MA_TX&SLA_RX/main.c 
  * Version: V1.3.2
  * Date   : 2021-08-08
...
/** @addtogroup Poll_PEC_MA_TX&SLA_RX
  * @{
  */ 
...
int main(void)
{
  ...
  printf("Polling(PEC) - Master Transmitter & Slave Receiver.\r\n");	  
  ...
  while(1)
  {
    ...
    if((I2C_Status = I2C_Master_Transmit(I2C_PORT, I2C_SLAVE_ADDRESS7, tx_buf, BUF_SIZE - 1, 10000)) != I2C_OK)
    {
    ...
    /* Timeout is set to 10S  */
    if((I2C_Status = I2C_Slave_Receive(I2C_PORT, rx_buf, BUF_SIZE, 10000)) != I2C_OK)
    {
      /* Error_Handler() function is called when error occurs */ 
      Error_Handler(I2C_Status);
    }
